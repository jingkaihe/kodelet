...
The current implementation preserves the design's direct-host constraints: no worktrees, containers, micro-VMs, filesystem snapshots, process namespaces, or network namespaces. The [daemon-first migration](daemon-first-design.md) supersedes this document's historical direct-local CLI/default assumptions: ordinary CLI/ACP execution is daemon-backed, and `run --runner` is supported. The runner provisions every run through an `ExecutionInstanceProvider`, and the built-in provider returns a fresh lifecycle handle backed by the conversation's selected host directory, defaulting to the registered startup workspace. Concurrent runs therefore receive separate run state and extension runtimes but intentionally share the host filesystem, processes, network, and ports.
...
The runner negotiates capabilities at registration, then discovers its manifest before the first ready heartbeat. Until then, WebSocket liveness guards the connection; application-heartbeat expiry begins after readiness. Discovery-only probes do not start extension session lifecycle; the first real run supplies the call context for `session.start` and `resources.discover`. The initial cold probe is bounded by the connection context rather than the short refresh deadline. Periodic probes run asynchronously with a bounded context, so slow discovery cannot suppress heartbeats. Only one probe or `run.open` owns the fair snapshot gate at a time. A concurrent `run.open` waits according to its caller context, while periodic probes reuse the last successful digest when the gate is occupied.
...
The terminal protocol uses `workspace.terminal.open`, `workspace.terminal.read`, `workspace.terminal.input`, and `workspace.terminal.resize`. The runner owns one persistent current PTY session per canonical directory, retaining at most eight directories and reclaiming exited sessions when another directory needs a slot. `open` validates its optional `cwd` on the runner, creates or reattaches to that directory's session, and returns absolute replay and write cursors. `read` performs a bounded long poll from an absolute cursor; the runner retains a bounded replay buffer and reports when earlier output was truncated. Input and resize calls address the returned session ID, not whichever directory was most recently opened. Browser WebSocket attachment and replay framing remain control-plane concerns. The control plane caps concurrent browser attachments per runner so terminal long polls cannot consume the runner connection's general request concurrency. Shutdown closes all retained directory sessions.

Runners advertising `workspaceDiscovery` implement `workspace.discover` and `workspace.cwdHints`. Discovery resolves `cwd` and `environmentProfile` on the runner without reserving a run or starting a provider turn; command discovery returns the resolved directory, profile, manifest digest, and commands. Targeted probes do not replace the runner's startup manifest digest. Directory hints resolve query paths relative to the runner startup directory or an explicit base and return canonical runner-host paths. The authenticated HTTP slash-command and CWD-suggestion endpoints accept `runnerId`, `cwd`, and `environmentProfile`, or a `conversationId` whose stored affinity supplies those values. Unsupported, disconnected, or invalid runner targets return errors instead of falling back to daemon-local discovery. Live registry connection/generation fencing applies to these calls.

Detaching a browser WebSocket does not stop the runner-owned PTY. On runner shutdown, cleanup sends `SIGHUP` to the captured foreground and shell process groups, closes the PTY, waits briefly, then sends `SIGKILL` and returns after a fixed deadline. Cleanup is best effort: it does not enumerate arbitrary descendants that moved into another process group or session, and any incomplete cleanup is reported while the workspace lock is still released. Ctrl+C, Ctrl+Z, and similar controls travel through normal PTY input.
...
Omitted authentication modes preserve the legacy behavior by generating distinct web/API and runner tokens. Authentication modes may be selected with `kodelet serve` flags or the trusted user-level `serve` configuration namespace; repository configuration cannot set that namespace, and explicit flags override YAML. Configured tokens are retained in user-only trusted configuration and are not echoed at startup, while newly generated tokens are displayed once. Token and enrollment authentication are mutually exclusive runner modes; enrollment mode does not accept the legacy shared runner token.
