...
TypeScript `ext.registerProfile(...)` and Python `ext.register_profile(...)` require `provider` and `model` and return the flat name unchanged. Names are 1–128-character ASCII slugs; `default` is an ordinary name, and conflicting configured or registered names are rejected. Profiles are scoped to the authenticated caller and runner. ACP clients target `ctx.runnerId` (Python: `ctx.runner_id`); workspace discovery registers installed extension profiles under the client's own identity before selecting one, so a child does not need the Web UI parent's credentials. Direct chat requests also discover missing registrations in their selected workspace. Session-only profiles still require registration under the selecting caller; they cannot be discovered from installed runner extensions.
...
A presentation frame is `{ "sequence": 1, "lines": [...] }`. Each line is either a plain string or `{ "spans": [{ "text": "...", "style": { ... } }] }`; supported style fields are `foreground`, `background`, `bold`, `dim`, `italic`, `underline`, `strikethrough`, and `reverse`. The TUI strips embedded ANSI/control sequences from extension text and accepts `#RRGGBB` colors; the Web UI renders safe named ANSI colors and hex colors. Multi-line widgets above or below the composer start folded in both UIs, with the first line as a clickable summary. Frame updates preserve the fold state. The TUI also supports `Ctrl+O` and mouse-wheel scrolling within ten visible rows per placement; the Web UI bounds scrolling to expanded content.
