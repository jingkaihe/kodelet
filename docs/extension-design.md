...
### Extension-owned execution presets

`extension.initialize` may return `profiles: [{name, options?, systemPromptPath?, systemPrompt?}]`. TypeScript registers these with `ext.registerProfile`; Python uses `ext.register_profile`. `options` is the same strict, credential-free `ExecutionOptions` schema used by daemon requests. Prompt content and prompt paths are mutually exclusive; the runner reads paths relative to the owning extension directory, then advertises content in its pinned environment manifest. Registration is scoped to extension identity and process generation, capped at 32 presets, and never creates a global daemon model profile.

Typed `allowedTools` selects from the tool catalog independently of default tool mode; omission keeps the default presentation. Likewise, the raw filesystem-search setting is a preference, while typed `enableFSSearchTools: false` is a mandatory restriction. Ordinary requests and child presets obey the same explicit policy ceilings; children additionally inherit the parent's explicit `agent.init` tool-list patch, including deny-all.

Children do not inherit the parent's `allowed_reasoning_efforts`; presets and requests may choose any provider-supported effort. Other policy ceilings and saved reasoning settings remain enforced.

The owning extension tool calls `kodelet.child.start` with `{requestId, profile, message, contextMode?, resume?, options?, systemPrompt?, cwd?, leaseId?}`. The runner supplies the observed run/tool/extension identity and process generation over its authenticated `child.start` RPC. The registry validates current runner connection/generation and pinned active tool ownership, resolves the preset from the parent environment, and applies the parent's policy ceiling through ordinary central chat validation. The child uses the existing persisted chat loop with distinct conversation/run IDs and parent metadata; no provider or administrative credentials are exported to the extension. Persistence and runner affinity precede successful admission acknowledgement. A repeated request ID within the same capability returns the same child, including while preparation is in flight; different input under that ID is rejected.

`contextMode` defaults to `fresh`. `fork` snapshots the currently authenticated live parent through the provider's existing safe-fork cleanup, preserving native history while removing unresolved tool calls; it never accepts a caller-supplied source ID or publishes a temporary unrestricted conversation. Fork requires the originating active tool, even when that tool retains the new child's lease. `resume` is an existing child conversation ID and cannot be combined with `fork`. Followups require new current tool/lease authority and a new request ID, preserve the conversation ID, and get a new run ID. Durable provenance must match the parent conversation, extension, runner installation, environment profile and preset; working directory and system prompt cannot change. Saved model/preset/prompt remain authoritative over changed extension registrations, and current mandatory restrictions can only narrow the saved policy. Child and ordinary admissions share the durable active-turn receipt constraint; resume uses a record-version comparison so competing submissions cannot overwrite history. Failed, cancelled or interrupted children can be explicitly resumed with new authority, but a fork ID or an old lease alone is never restart authority.

`kodelet.child.read` and `kodelet.child.cancel` take `{childId, childRunId, leaseId?, after?}` and are scoped to the same capability. Older fresh-child SDKs may omit `childRunId` only for an unambiguous non-followup handle; a stale exact handle can never address a newer run. Retained handle requests use the extension process's connection lifetime, never an unrelated pending foreground handler; the runner pins each lease's child-authority origin run across reattachment. Parentless starts cannot borrow an active handler's authority: they require an already activated retained grant, and forks still require an explicit active originating tool. Results contain the durable identities, `done`, optional `cancelled`/`error`/`output`, and sequenced events. Progress retains the latest 128 events, with bounded event text; `after` is a cursor, not a promise of complete replay. At most 64 submissions are retained per capability. Cancellation propagates to the exact child provider context without cancelling siblings.

`kodelet.child.steer` takes `{childId, childRunId, message, requestId, leaseId?}` and never starts a turn. It returns `{outcome: "injected"}` for queued guidance on the exact running child, or `{outcome: "promptRequired", reason: "noRunningTurn"}` after completion. Each child retains at most 128 steering request receipts; retries with the same ID and input return the original result, while conflicting reuse is rejected. Guidance is scoped to its child run. Acknowledged guidance not consumed before completion transfers only on a validated explicit followup of that owned child, not to an unrelated ordinary turn.

An active tool owns foreground children only until its handler ends. A real, activated background lease can retain authority beyond successful parent completion; its first child must be admitted from the active tool, and its non-renewing maximum lifetime is one hour. Provisional opening leases are rejected. Release, parent cancellation, runner disconnection/replacement, extension failure, and shutdown revoke authority and cancel unfinished children. Explicit lease close acknowledges success only after bounded cancellation/drain of admitted and preparing children, including a start whose reply was lost. Timeout or transport failure leaves release unconfirmed and retryable; the SDK must not infer that an uncertain start was never admitted. Runner-local submission fencing and bounded connection-lifetime revocation tombstones reject delayed starts after acknowledged close; disconnect cleanup remains best effort. At most 1024 retained child authorities/revocation entries exist per registry, and exhausted capacity fails closed until reconnection clears that connection's entries. Durable conversations remain resumable by authorized daemon clients with the saved preset/prompt and runner affinity; transient runner capabilities and request caches do not survive restart and are never silently replayed. Background child usage belongs to its own conversation; foreground child usage aggregates only that execution's delta into the parent, not prior followup usage.
...
Runner leases acquired during initialization or `session.start` are provisional: they reserve capacity but become background retention and child-execution authority only after `run.open` successfully finishes environment discovery. A failed or canceled opening revokes its provisional leases and closes its resources. Extensions must tolerate this cleanup even after acquisition returned a handle; acquisition does not acknowledge a successful open. The runner allows at most 64 leases in total, including provisional leases, with descriptions limited to 1,024 bytes.
...
Daemon-backed native surfaces and transcript appends go only to the selected capable client, with acknowledgements bounded to five seconds. Surface input and resize acknowledgements use an opaque route bound to the client owner, runner connection generation, extension process generation, conversation, and surface opening. Output frames remain visible across individual tool/command returns, but surfaces close on owner change, disconnect, cancellation, or execution completion. A retained worker must explicitly reopen its surface during a later execution; UI never acquires a worker lease or restores a surface transparently. Late acknowledgements, input, resize, and output from revoked owner epochs cannot recreate interactive UI. Each execution allows at most 32 open surfaces and 64 pending native requests; individual native requests are limited to 512 KiB.

Client takeover refreshes availability in the existing extension process through `kodelet.ui.capabilities` notifications containing `widgets`, `surfaces`, and `transcript` booleans. SDKs keep this override per persistent RPC client without changing the initialization snapshot. Host-initiated surface cleanup sends `extension.ui.surface.closed` with `scopeId`, `id`, and `openSequence` (the original open frame's sequence); SDKs dispose only that opening's handle, so a delayed closure cannot close a replacement. Capability refresh and cleanup never initialize or restart an extension. These notifications complement process-generation cleanup rather than promising durable UI state.
