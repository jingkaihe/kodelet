...
| `codex login/status`, `copilot-login` | Unconditional daemon device login and sanitized connection status using existing admin APIs. Browser launch is presentation-only; the daemon polls/exchanges and stores credentials. | Remote Codex/Copilot logout is explicitly unavailable. Stop the daemon, use `host codex logout` or `host copilot-logout` on its host, then restart. Detailed Codex usage is explicit `host codex status`; real device authorization remains an external acceptance gate. |
| `profile current/list/show/use` | `current/list/show` read daemon-advertised model profiles and reasoning settings. Top-level `use` rejects without effects; full configuration inspection/mutation is explicit `host profile`, followed by a daemon restart. | No live daemon configuration mutation or credential-bearing configuration export. Runner environment profiles remain a separate namespace; host-local edits never silently configure another daemon. |
...
| `recipe list/show` | Explicit `host recipe list/show` operator commands on the runner host in the intended workspace. Old top-level paths retain their flags but fail with migration guidance before discovery or rendering. | Listing starts local extensions and their lifecycle; showing renders file-backed templates and may execute commands locally. No remote preview or dynamic-extension-recipe rendering is advertised. Ordinary `run -r` remains daemon/runner-owned. |
| `extension list/inspect` | Explicit `host extension list/inspect` filesystem-only operator commands; no subprocess startup. Old top-level paths fail with migration guidance, not an implicit host fallback. | Reports installed files on the invoking host, not selected-runner policy, live registrations, or an active conversation's manifest. No new remote inspection API or plugin/setup changes. |
...
**Completed W7 host-inspection disposition slice:** recipe and extension inspection handlers are reparented under `host`; legacy commands parse their existing flags and return exact migration guidance before effects. This is an explicit operator surface, not remote inspection parity. CLI subprocess tests with an unusable conversation-store path and poisoned extension/template markers prove legacy rejection, help-only behavior, filesystem-only host extension inspection, and the explicitly requested host recipe startup/rendering effects. Explicit extension startup may create extension-owned data, but does not initialize conversation storage. Full command tests, focused race tests, and `mise run lint` pass. Runner-backed browser/TUI/ACP command discovery is unchanged; broader release completion is not implied.
...
| Ownership and migration | Credential-free/store-free client traps, default selection, validated adoption, no-daemon rejection, explicit host commands, and local-fallback source audit pass. `setup` remains a concise daemon-host configuration command. |
