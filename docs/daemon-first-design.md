...
| Persistent UI | [Native UI bridge](../pkg/chat/persistent_ui.go) routes owner-fenced surfaces, transcript, input, and resize; passive widgets remain independently mirrored. | Maintain the documented native/browser capability boundary and distinguish protocol acceptance from visual client acceptance. |
...
8. One active run per conversation remains enforced; extensions own background leases and the ACP sessions they launch.
...
Tool-free model extraction and workspace-capable subagents are different operations. The former needs central model execution and temporary input, not necessarily another workspace lease. The latter uses a normal ACP conversation with client authentication, runner context, and extension-owned cancellation.
...
When multiple clients attach, only the capable submitting client should own a blocking prompt or interactive surface for that turn. Passive observers can continue receiving conversation events. Disconnect, background lease release, and run termination must not leave hanging requests or stale interactive resources. Persistent extension UI does not imply durability across daemon restarts.
...
**Phase-one revision, September 8, 2026:** Remove scoped child execution and extension profile registration from Go and both SDKs. Use ordinary ACP sessions with typed options, inline prompt hooks, named forks, and normal client credentials. The SDK owns transport cleanup; extensions own their sessions and background leases. See [ACP agent sessions](extension-design.md#acp-agent-sessions).

Regression coverage checks both runner placements, live broadcasting, named fork titles, and explicit search tools under a patch-mode parent. `code_search` keeps an advisory turn budget, read-only tools, and no skills; inline instructions require extensions to remain enabled. SDK/extension releases must be coordinated. Runner-targeted authorization remains phase two.
...
The runner lifetime contract is implemented and documented in [Background extension work](extension-design.md#background-extension-work): leases acquired during initialization or `session.start` are provisional until successful `run.open` and share the runner-wide limit of 64. A lease retains resources, not client execution authority. Ordinary close retains explicitly leased workers; failed opening, explicit cancellation, process-generation failure, final release, and shutdown revoke the corresponding ownership. Retained extension settings are pinned until release; disposable discovery never starts session lifecycle or retains workers. Persistent extension storage remains extension-owned and is not deleted when a runtime ends.
...
Extensions own their ACP session and background-task cleanup. Test reload/removal and cancellation without changing unrelated sessions or treating host lease retention as execution authorization.
...
Each turn's capable submitting client owns interactive UI. Other clients can watch, steer, and stop, but cannot answer extension prompts. Disconnecting dismisses UI without cancelling execution; reconnecting does not restore it mid-turn. Prompts use fresh, owner-bound response IDs, a five-minute timeout, and bounded response writes. Tests cover observer rejection, reused IDs, stale replies, disconnect without handoff, and queued-dialog cleanup.
...
Native remote chat now connects runner surface open/frame/close, transcript appends, and passive widget snapshots to the existing TUI renderer. Only the capable submitting client receives interactive output; input and resize return through generation- and opening-fenced routes with bounded acknowledgements. Browser surfaces remain unavailable. Process/connection loss, cancellation, and run termination revoke interactive resources without retaining a worker just for UI; leased background widgets remain independent. Both SDKs refresh UI capabilities when a new run starts and discard closed surface handles using the original opening sequence, including closure while opening and delayed closure after replacement. Extensions must reopen surfaces on a later execution; transparent recovery is not claimed.
...
`TestNativeUIReleaseAcrossRunnerPlacements` runs the same multi-client fixture with an embedded runner and a separate standalone runner process, using real WebSocket runner transport, stdio reverse RPC, native chat HTTP transport, and a browser-capability NDJSON subscriber. It checks observer prompt denial, native surfaces/input/resize/transcript, unavailable browser surfaces/transcript, disconnected-owner and completed-run cleanup, client detachment without canceling execution or selecting another owner, new-turn submitting-client UI, and background widget delivery after foreground completion. Runner connection loss also reaps the retained worker; `TestBackgroundConnectionLossRevokesRetainedWorkers` checks that reconnect starts a fresh process without deleting persistent data. This fixture uses deterministic central orchestration and a headless native host, not a live model, interactive terminal, or visual browser automation. Those visual/client release checks remain separate; no transparent UI recovery or transient-event replay is claimed.
...
| ACP agentic session | `code_search` runs without a preconfigured daemon profile; inline instructions, read-only tools, progress/cancellation, named forks, and normal conversation broadcasting survive the supported lifecycle. |
...
This historical record predates the September 8 ACP restoration. The scoped-child-specific gates below are superseded by ordinary ACP delegation coverage; they are not verification of a retained scoped execution API.
...
Keep tool-free helpers scoped to their active run/tool. Subagents and code search use normal ACP client credentials in phase one. Phase two may allow client operations targeting the authenticated runner, with conversation ownership and affinity checks; caller-supplied runner IDs and shared registration tokens are insufficient. Unrelated runners, conversations, and administrative operations remain forbidden.

Credential lifetime and revocation must remain separate from background leases. Reuse ordinary conversation admission and streaming, not another child execution path.

**Owner and completion:** Embedded runner provisioning remains workstream 2. Runner-targeted client identity and conversation ownership are deferred phase-two work; no new authorization is granted by the phase-one ACP restoration.
...
Only the capable submitting client owns interactive UI. Without one, return unavailable/dismissed rather than selecting an observer. Other clients can still steer and stop.

Disconnect dismisses prompts and revokes input without cancelling execution. Reattachment and late replies cannot restore UI mid-turn. Each new turn uses its submitting client; passive widgets may be mirrored to observers.

**Owner and completion:** Workstream 5, with release scope agreed in 7. Publish a per-client capability/release matrix and test TUI-plus-browser attachment, submitting-client ownership for each turn, stale responses, and loss of the owning client without automatic handoff.
