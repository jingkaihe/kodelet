...
In the Web UI, choose a runner from the **Environment** field when creating a conversation. **Runner profile** is optional; leaving it blank uses the runner's base configuration. Kodelet saves this selection with the conversation and reuses it for later turns. Compatible runners show the terminal and Git diff in the right-side workspace panel. Terminal sessions remain available when the panel is closed, the browser reconnects, or the terminal is opened in a pop-out window; terminal access requires the `terminal` or `admin` role. Older runners hide unsupported controls. Remote conversations do not offer control-plane CWD suggestions or slash-command discovery, but chat commands still run on the selected runner. Recipe changes take effect on the next run.
