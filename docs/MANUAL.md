...
For a deployment where the control plane must not host a workspace environment, start it with:

```bash
kodelet serve --disable-control-plane-workspace
```

The equivalent trusted setting is `serve.disable_control_plane_workspace: true`. This keeps the provider loop, conversation persistence, runner registry, and client APIs on the control plane, but requires a workspace runner for every executable conversation. It disables the control-plane-local chat fallback, terminal, Git diff, CWD suggestions, and server-side workspace slash-command and extension discovery. `--cwd` and `serve.cwd` cannot be combined with this mode. Existing local conversations remain browseable but cannot execute without an explicitly selected runner; the Web UI removes the local environment choice and presents such conversations as read-only.
...
When the Web UI already has a persisted conversation open, it keeps an idle control-plane stream attached to that conversation. A turn started from `kodelet chat --server ... --resume CONVERSATION_ID` therefore appears live in the Web UI, including turns that execute through `--runner`; when the control-plane workspace is enabled, the same behavior applies to server-local execution without `--runner`. A standalone local TUI does not participate because it bypasses the control plane.
