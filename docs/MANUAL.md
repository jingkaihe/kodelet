...
kodelet chat --server https://kodelet.example  # use control-plane conversations and execution
kodelet chat --server https://kodelet.example --resume CONVERSATION_ID
...
kodelet auth status --server https://kodelet.example
kodelet auth logout --server https://kodelet.example
```

`login` prints a short code and the generic verification URL, opens that page when possible, and waits for approval. The user must manually enter the displayed code; Kodelet does not put it in the browser URL, which adds a small human-presence check against one-click remote-phishing links. The verification page requires a genuine OIDC browser session and shows the code and client metadata again before approval; if necessary, the server first completes its normal authorization-code flow with PKCE, state, nonce, and ID-token verification. After approval, the server activates a Kodelet-owned opaque bearer credential carrying a snapshot of that principal's roles, stores only its hash, and returns no Google, GitHub, or other provider access token to the client. Use `--no-browser` to print the URL without attempting to launch a browser. `logout` revokes the current server-side credential before deleting local state, and `status` validates the stored credential and displays its principal, roles, and expiry.
...
Same-origin Web UI requests do not require CORS. Browser requests from loopback origins are allowed by default for local development. To allow additional browser origins, pass `--cors-origins` with a comma-separated list:

```bash
kodelet serve --cors-origins https://app.example.com,https://admin.example.com
```

For trusted local-only use, disable both human and runner authentication with:
...
```bash
cd ~/src/kodelet
kodelet runner enroll \
  --server https://kodelet.example \
  --name kodelet-gpu
```

The command generates an Ed25519 key pair, stores the pending enrollment under `~/.kodelet/runners`, prints a short user code and the generic verification URL, opens that page when possible, and polls until a `runner-admin` or `admin` approves or denies it. The approver must manually enter the displayed code before reviewing the runner metadata and public-key fingerprint; Kodelet does not put the code in the browser URL. Pending enrollment survives a CLI restart until its ten-minute expiry. Use `--no-browser` to suppress browser launch. Use `--replace` when the workspace already has a local credential; the browser approval page also requires explicit confirmation before the old server-side credential is revoked. An approved replacement may target a currently connected enrolled generation: the control plane revokes its credential, marks its active work lost, and disconnects it so the replacement can take effect.
...
kodelet runner list --server https://kodelet.example --json
kodelet runner inspect kodelet-gpu --server https://kodelet.example
...
Across hosts, runner and remote TUI connections require HTTPS/WSS; plain HTTP is accepted only for loopback servers. Control-plane URLs are canonicalized for endpoint construction and local registration-cache identity, so equivalent host casing, trailing dots, default ports, and base-path spellings do not create duplicate local registrations. The built-in runner executes directly in its workspace with the runner process's host permissions, permits multiple active top-level runs while retaining one active run per conversation, and is not a sandbox. It does not create worktrees, containers, micro-VMs, filesystem snapshots, process namespaces, network namespaces, or isolated port spaces, so concurrent runs share files and host resources. Each run is created and cleaned up through an execution-instance provider, but the built-in provider currently returns a fresh lifecycle handle to the same workspace; an isolated provider and its durability/conflict model remain future work behind the unchanged agent-environment protocol.

Runner cancellation and resource cleanup are bounded and scoped to the affected run. If an operation or run-private resource does not stop within the cleanup deadline, that close reports an error and the control plane records the affected run as lost, releases its active lease, and keeps unrelated runs and the shared connection available. The same conversation may start another run immediately; because the direct-workspace runner intentionally shares files, processes, and ports, any real conflict left by incomplete cleanup is surfaced by that later run rather than preemptively blocking it. A control-plane `run.open` transport failure is reconciled with `run.close`; if cleanup cannot be confirmed, only that opening run is recorded as lost.
...
reduce end-to-end latency. Sequential Responses requests reuse one connection;
interactive chat sessions retain that connection across compatible conversation
turns. After a successful response, Kodelet continues with `previous_response_id`
and sends only new input items when the request is a compatible extension of the
previous one.

Kodelet still sends `store: false` and retains complete local conversation state.
If the socket closes, reaches its server-enforced lifetime, or its connection-local
response state is unavailable, Kodelet reconnects and retries with the complete
local input context. Only one response is in flight on each socket. If WebSocket
