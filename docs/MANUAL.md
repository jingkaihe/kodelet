...
The Web UI and a server-backed TUI keep idle control-plane streams attached to persisted conversations they have open. A turn started from `kodelet chat --server ... --resume CONVERSATION_ID` therefore appears live in the Web UI, and a turn started from the Web UI appears live in a TUI that has opened the same conversation through `--resume`, `--follow`, or `/sessions`. This also applies to turns that execute through `--runner`; server-local turns participate when the control-plane workspace is enabled. The TUI refreshes canonical persisted history when an observed turn completes, so attaching partway through a turn is reconciled after completion. A standalone local TUI does not participate because it bypasses the control plane.
