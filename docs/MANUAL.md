...
- [Usage Modes](#usage-modes)
  ...
  - [Web UI Server](#web-ui-server)
    - [Control-plane container](#control-plane-container)
...
Before the first message, use `Ctrl+T` to choose a profile, `/model` to choose its main model, and `Ctrl+Y` to choose a reasoning effort. Type to filter the model picker, use arrow keys to navigate, and press `Enter` to select or `Esc` to cancel; `/model <id>` selects a listed model directly. These settings are fixed after the conversation starts. The model picker uses the selected profile's provider/platform catalog and configured custom models, retaining its configured model even when it is not in the catalog. Both pickers keep the selected model first and sort the rest in descending numeric-aware order; model IDs do not establish release order across model families. Model selection overrides only the main model for the conversation: it does not edit the profile, change its helper model, credentials, prompt, or tools, or require a server restart. Switching profiles resets the model to that profile's default.
...
The new-conversation dialog also provides a **Model** picker scoped to the selected model profile. It has the same conversation-only override behavior as `/model` in the TUI; existing conversations retain their saved model. After applying a selection, the composer displays `model:<id>`; reopened conversations show their saved model rather than the profile's current default. Model choices come from the daemon, including when the browser or TUI runs on another machine. Built-in Anthropic, OpenAI, and Codex model lists ship with Kodelet; Copilot uses its cached provider catalog. Custom endpoints use configured model entries rather than assuming the standard provider's catalog, and a listed model does not guarantee account access.
...
The TypeScript SDK rejects unsupported shortcut identifiers during registration, and the host validates registrations again for extensions that implement the JSON-RPC protocol directly. Invalid registrations are not advertised in the TUI shortcuts dialog.

The native TUI reserves `ctrl+c`, `ctrl+d`, `ctrl+l`, and `ctrl+j`; conflicting registrations are skipped with a warning. An extension shortcut may override other built-in composer bindings such as `ctrl+r`, also with a warning. If multiple extensions register the same normalized shortcut, the later-loaded extension wins and Kodelet reports the conflict. A shortcut does not run while a dialog, picker, slash-command completion list, history search, or capturing extension surface owns keyboard focus.
