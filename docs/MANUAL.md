...
Use `/sessions` to open a searchable conversation picker. The picker includes saved conversations and conversations already open in the current TUI, keeps timestamps and working-directory names in aligned columns, and uses compact row indicators for the current (`›`), running (animated spinner), unread (`•`), and input-required (`!`) states without displaying conversation ID suffixes. It preserves each conversation's draft and scroll position when switching. Use `/new` to start another conversation. One run may be active per conversation, while different conversations can run in parallel; `Enter` queues steering and `Esc` or `Ctrl+C` cancels only the currently selected conversation. If a background run requires extension input, the TUI marks it in the picker and shows a notification so you can switch to it and respond.
