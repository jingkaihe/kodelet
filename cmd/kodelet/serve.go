...
func init() {
	...
	serveCmd.Flags().String("runner-auth-token", defaults.RunnerAuthToken, "Token accepted only by runner registration; generated by default")
...
func getServeConfigFromFlags(cmd *cobra.Command) *ServeConfig {
	...
	if runnerAuthToken, err := cmd.Flags().GetString("runner-auth-token"); err == nil {
...
func validateServeConfig(config *ServeConfig) error {
	...
	if config.SkipAuth && config.RunnerAuthToken != "" {
		return errors.New("--runner-auth-token cannot be used with --skip-auth")
	}
	...
	if config.AuthToken != "" && config.RunnerAuthToken != "" && config.AuthToken == config.RunnerAuthToken {
		return errors.New("--runner-auth-token must differ from --auth-token")
	}
...
func runServeCommand(ctx context.Context, config *ServeConfig) {
	...
	runnerAuthToken := strings.TrimSpace(config.RunnerAuthToken)
	...
	if !config.SkipAuth && runnerAuthToken == "" {
		var err error
		runnerAuthToken, err = webui.NewAuthToken()
		if err != nil {
			presenter.Error(err, "failed to generate runner auth token")
			os.Exit(1)
		}
	}
	...
	if runnerAuthToken != "" {
		presenter.Info(fmt.Sprintf("Runner authentication token: %s", runnerAuthToken))
	}
