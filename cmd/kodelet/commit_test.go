...
func TestEditMessageWithExplicitClientEditor(t *testing.T) {
	editor := writeTempScript(t, "editor", `#!/bin/sh
printf 'fix: edited message
' > "$1"
`)
	t.Setenv("PATH", t.TempDir())
	assert.Equal(t, "fix: edited message\n", editMessageWithEditor(t.Context(), "feat: original", editor))
}
...
exit 7
`)
	assert.Equal(t, "feat: original", editMessageWithEditor(t.Context(), "feat: original", editor))
}
