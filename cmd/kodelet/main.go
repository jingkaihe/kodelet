...
func main() {
	...
	rootCmd.PersistentFlags().Int("thinking-budget-tokens", 4048, "Enable Anthropic thinking; used as budget_tokens on non-adaptive Claude models (overrides config)")
	...
	rootCmd.PersistentFlags().String("reasoning-effort", "medium", "Reasoning effort for supported models (provider-specific; e.g. OpenAI none|minimal|low|medium|high|xhigh, Anthropic low|medium|high|xhigh|max)")
