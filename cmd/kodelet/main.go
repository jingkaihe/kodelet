...
func init() {
	...
	viper.SetDefault("thinking_tokens", 4048)
...
func main() {
	...
	rootCmd.PersistentFlags().Int("thinking-tokens", 4048, "Maximum tokens for thinking capability (overrides config)")
	...
	viper.BindPFlag("thinking_tokens", rootCmd.PersistentFlags().Lookup("thinking-tokens"))
