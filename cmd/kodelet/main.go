...
func init() {
	...
	viper.SetDefault("openai.platform", "")
	viper.SetDefault("openai.service_tier", "")
	viper.SetDefault("openai.base_url", "")
	viper.SetDefault("openai.api_key_env_var", "")
	viper.SetDefault("openai.manual_cache", false)
	viper.SetDefault("openai.enable_search", true)
	viper.SetDefault("openai.websocket_mode", true)
	viper.SetDefault("anthropic.platform", "")
	viper.SetDefault("anthropic.base_url", "")
...
func main() {
	...
	rootCmd.PersistentFlags().String("reasoning-effort", "medium", "Reasoning effort for supported models (provider-specific; e.g. OpenAI none|minimal|low|medium|high|xhigh, Anthropic adaptive none|low|medium|high|xhigh|max)")
