...
func TestRemoteProviderAndProfileProcessesNeverUseClientState(t *testing.T) {
	for _, test := range []struct {
		...
		{"set-default", []string{"anthropic", "accounts", "default", "work"}, "", "Daemon account updated", false, 1},
		{"rename", []string{"anthropic", "accounts", "rename", "work", "renamed"}, "", "Daemon account updated", false, 1},
		{"remove", []string{"anthropic", "accounts", "remove", "work"}, "", "Daemon account updated", false, 1},
		{"usage", []string{"anthropic", "accounts", "usage", "work", "--json"}, "", `"utilization": 0.25`, false, 1},
		{"logout", []string{"anthropic", "logout", "--no-confirm"}, "", "accounts removed from the daemon", false, 1},
		{"decline-logout", []string{"anthropic", "logout"}, "n\n", "Logout canceled", false, 0},
		{"oauth", []string{"anthropic", "login", "--no-browser", "--alias=named"}, "copied-code\n", "connected on the daemon", false, 2},
		...
		{"profile-missing", []string{"profile", "show", "local-only"}, "", "no local configuration fallback", true, 1},
		...
		{"codex-status", []string{"codex", "status"}, "", "Daemon Codex connected: true", false, 1},
		{"codex-login", []string{"codex", "login", "--device-auth", "--no-browser"}, "", "codex subscription connected on the daemon", false, 2},
		{"codex-callback-rejected", []string{"codex", "login", "--device-auth=false"}, "", "requires device authentication", true, 0},
		{"copilot-login", []string{"copilot-login", "--no-browser"}, "", "copilot subscription connected on the daemon", false, 2},
