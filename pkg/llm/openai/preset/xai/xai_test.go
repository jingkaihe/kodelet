...
func TestModels(t *testing.T) {
	...
	t.Run("ReasoningModels", func(t *testing.T) {
		...
		assert.Equal(t, expectedReasoning, Models.Reasoning, "Reasoning models should match expected list")
	})
	...
	t.Run("NonReasoningModels", func(t *testing.T) {
		...
		assert.Equal(t, expectedNonReasoning, Models.NonReasoning, "Non-reasoning models should match expected list")
	})
}

func TestPricing(t *testing.T) {
	require.NotNil(t, Pricing, "Pricing should not be nil")
	...
	t.Run("SpecificModelPricing", func(t *testing.T) {
		...
		imagePricing, exists := Pricing["grok-2-image-1212"]
...
func TestBaseURL(t *testing.T) {
	...
	assert.Equal(t, expectedURL, BaseURL, "BaseURL should match expected xAI API endpoint")
}

func TestModelCount(t *testing.T) {
	// Ensure we have the expected number of models
	assert.Len(t, Models.Reasoning, 3, "Should have 3 reasoning models")
	assert.Len(t, Models.NonReasoning, 2, "Should have 2 non-reasoning models")
	assert.Len(t, Pricing, 5, "Should have pricing for 5 total models")
}
...
func TestAPIKeyEnvVar(t *testing.T) {
	...
	assert.Equal(t, "XAI_API_KEY", APIKeyEnvVar, "APIKeyEnvVar should be XAI_API_KEY")
