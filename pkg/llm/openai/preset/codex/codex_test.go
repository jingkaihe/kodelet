...
func TestPricing(t *testing.T) {
	assert.Contains(t, Pricing, "gpt-6-astra")
...
func TestPricingForServiceTier(t *testing.T) {
	...
	assert.Equal(t, 0.000025, priority["gpt-6-astra"].Input)
	assert.Equal(t, 0.0000025, priority["gpt-6-astra"].CachedInput)
	assert.Zero(t, priority["gpt-6-astra"].CacheWriteInput)
	assert.Equal(t, 0.000125, priority["gpt-6-astra"].Output)
	assert.Equal(t, 272_000, priority["gpt-6-astra"].ContextWindow)
	...
	tests := []struct {
	...
	}{
		{
			...
			priority: llmtypes.ModelPricing{
				...
				Output:                 0.000025,
				LongContextInput:       0.00001,
				LongContextCachedInput: 0.000001,
				LongContextOutput:      0.0000375,
				LongContextThreshold:   272_000,
				ContextWindow:          272_000,
			},
		},
		{
			...
			priority: llmtypes.ModelPricing{
				...
				Output:                 0.00000125,
				LongContextInput:       0.0000005,
				LongContextCachedInput: 0.00000005,
				LongContextOutput:      0.000001875,
				LongContextThreshold:   272_000,
				ContextWindow:          272_000,
			},
		},
	...
	for _, tt := range tests {
		t.Run(tt.model, func(t *testing.T) {
			for _, tier := range []struct {
				...
				{llmtypes.OpenAIServiceTierFlex, tt.standard},
