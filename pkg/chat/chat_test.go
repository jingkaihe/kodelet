...
func TestChatMessageHandler_HandleUserMessageEmitsRenderableContent(t *testing.T) {
	...
	blocks, ok := sink.events[0].Content.([]ChatContentBlock)
	require.True(t, ok)
...
`)
	...
	prompt, expansion, err := ExpandSlashCommand(context.Background(), "/limited name=Web check this", workspace)
...
func TestTryWebExtensionCommandRoutesCommand(t *testing.T) {
	...
	result, handled, err := TryExtensionCommand(
		context.Background(),
		"/doctor verbose=true",
		runtime,
...
func TestWebSlashCommandRestrictionsApplyBeforeBuildingState(t *testing.T) {
	...
	ApplyFragmentRestrictions(context.Background(), &config, metadata)
...
`)
	...
	_, expansion, err := ExpandSlashCommand(context.Background(), "/limited", workspace)
	require.NoError(t, err)
	require.NotNil(t, expansion)
