...
func TestCentralModelHelperUsesFrozenToolFreeProvider(t *testing.T) {
	...
	goal := "Extract the evidence from the document"
	...
	assert.Equal(t, 1, conversationCount, "only the parent is persisted")
...
func TestCentralModelHelperCancelsProviderRequest(t *testing.T) {
	...
	case <-started:
	case err := <-done:
		require.FailNowf(t, "helper returned before starting the provider request", "%v", err)
