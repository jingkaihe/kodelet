...
func TestCentralModelHelperUsesFrozenToolFreeProvider(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(workspace, "AGENTS.md"), []byte("WORKSPACE_MUST_NOT_BE_DISCOVERED"), 0o600))
	t.Chdir(workspace)
	...
	provider := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		for _, forbidden := range []string{workspace, "WORKSPACE_MUST_NOT_BE_DISCOVERED", "PARENT_ONLY", "CUSTOM_WORKSPACE_PROMPT"} {
		...
		_, _ = fmt.Fprint(w, "data: "+`{"id":"helper","object":"chat.completion.chunk","model":"gpt-4o-mini","choices":[{"index":0,"delta":{"role":"assistant","content":"extracted"},"finish_reason":"stop"}],"usage":{"prompt_tokens":10,"completion_tokens":5,"total_tokens":15}}`+"\n\ndata: [DONE]\n\n")
	}))
	defer provider.Close()
	config := llmtypes.Config{
		Provider: "openai", Model: "gpt-4o", WeakModel: "gpt-4o-mini", WeakModelMaxTokens: 128,
		WorkingDirectory: workspace, AllowedTools: []string{"bash"}, Extensions: "parent runtime",
		Sysprompt: filepath.Join(workspace, "custom.tmpl"), SyspromptArgs: map[string]string{"project": "PARENT_ONLY"},
		OpenAI: &llmtypes.OpenAIConfig{
			BaseURL: provider.URL, APIKeyEnvVar: "KODELET_TEST_HELPER_KEY", APIMode: llmtypes.OpenAIAPIModeChatCompletions,
			Pricing: map[string]llmtypes.ModelPricing{"gpt-4o-mini": {Input: 0.01, Output: 0.02, ContextWindow: 10000}},
		},
	}
	...
	parent.Usage = &llmtypes.Usage{InputTokens: 7, CurrentContextWindow: 7, MaxContextWindow: 1000}
	...
	parent.Config.OpenAI.Pricing["gpt-4o-mini"] = llmtypes.ModelPricing{Input: 1, Output: 2}
	request := tooltypes.ModelHelperRequest{
		Operation: tooltypes.ModelHelperWebFetchExtract, URL: "https://example.com/source", Content: "DOCUMENT_ONLY", Prompt: "Extract the evidence",
	}
	for range 2 {
	...
	require.NoError(t, database.GetContext(t.Context(), &conversations, "SELECT COUNT(*) FROM conversations"))
	assert.Equal(t, 1, conversations, "only the parent is persisted")
	var storedMessages string
	require.NoError(t, database.GetContext(t.Context(), &storedMessages, "SELECT raw_messages FROM conversations WHERE id = ?", parent.GetConversationID()))
...
func TestCentralModelHelperCancelsProviderRequest(t *testing.T) {
	...
	parent, err := openaillm.NewOpenAIThread(llmtypes.Config{
		...
		OpenAI: &llmtypes.OpenAIConfig{BaseURL: provider.URL, APIKeyEnvVar: "KODELET_TEST_HELPER_KEY", APIMode: llmtypes.OpenAIAPIModeChatCompletions},
	})
	require.NoError(t, err)
	...
	go func() {
		_, err := tooltypes.RunModelHelper(ctx, tooltypes.ModelHelperRequest{
			Operation: tooltypes.ModelHelperWebFetchExtract, URL: "https://example.com", Prompt: "Extract title", Content: "document",
		})
		done <- err
	}()
	select {
