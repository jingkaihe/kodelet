...
func TestImageArtifactRunnerRoundTrip(t *testing.T) {
	...
	config.EmbeddedRunner.ServiceOptions.EnvironmentFactory = func(cwd string, runtime *extensions.Runtime) agentenv.Environment {
		...
		return &embeddedTestEnvironment{Environment: local, execute: func(ctx context.Context, request agentenv.ToolRequest, updates agentenv.ToolUpdateSink) (agentenv.ToolExecution, error) {
			...
			structured.Attachments = []tooltypes.ToolAttachment{{Type: "image", Path: imagePath, Alt: "Generated test pattern"}}
	...
	_, err = server.runnerRegistry.OpenRun(t.Context(), runnerID, protocol.RunOpenParams{RunID: "image-run", ConversationID: "image-conversation"})
	require.NoError(t, err)
	controller := artifactController{RemoteController: server.runnerRegistry, server: server, conversationID: "image-conversation", config: llmtypes.Config{Provider: "openai", Model: "gpt-4.1"}}
	generated, err := controller.ExecuteTool(t.Context(), runnerpayload.ToolExecuteParams{RunID: "image-run", ToolCallID: "generate", Name: "generate_test_image", Input: json.RawMessage(`{}`)}, nil)
	require.NoError(t, err)
	...
	relative := runnerpayload.ToolResult{Structured: tooltypes.StructuredToolResult{
		Attachments: []tooltypes.ToolAttachment{{Type: "image", ArtifactID: attachment.ArtifactID, ViewURL: "file://" + path}},
	}}
	...
	require.NoError(t, err)
	viewed, err := controller.ExecuteTool(t.Context(), runnerpayload.ToolExecuteParams{RunID: "image-run", ToolCallID: "view", Name: "view_image", Input: input}, nil)
	require.NoError(t, err)
	...
	_, err = server.runnerRegistry.OpenRun(t.Context(), runnerID, protocol.RunOpenParams{RunID: "other-image-run", ConversationID: "other-image-conversation"})
	require.NoError(t, err)
	otherController := artifactController{RemoteController: server.runnerRegistry, server: server, conversationID: "other-image-conversation", config: controller.config}
	denied, err := otherController.ExecuteTool(t.Context(), runnerpayload.ToolExecuteParams{RunID: "other-image-run", ToolCallID: "view-other", Name: "view_image", Input: input}, nil)
	require.NoError(t, err)
	...
	failed, err := controller.ExecuteTool(t.Context(), runnerpayload.ToolExecuteParams{RunID: "image-run", ToolCallID: "missing", Name: "generate_test_image", Input: json.RawMessage(`{"missing":true}`)}, nil)
	require.NoError(t, err)
	...
	raw, err := json.Marshal(runnerpayload.ArtifactRequest{RunID: "image-run", ToolCallID: "view", ArtifactID: attachment.ArtifactID})
	require.NoError(t, err)
	_, rpcErr := server.HandleRunnerArtifactRequest(t.Context(), runnerregistry.UIRequestIdentity{RunnerID: runnerID, ConnectionID: runner.ConnectionID, Generation: runner.Generation}, runnerpayload.MethodArtifactResolve, raw)
	...
	require.NoError(t, store.Save(t.Context(), convtypes.ConversationRecord{
		ID: "image-conversation", Provider: "anthropic", RawMessages: json.RawMessage(`[]`),
	...
	assert.Equal(t, "https://new.example/i/"+attachment.ShortCode, server.decorateImageAttachments(saved.ToolResults["generate"]).Attachments[0].ViewURL)
...
func TestImageArtifactUploadCancellationInterruptsStalledBody(t *testing.T) {
	...
	require.Eventually(t, func() bool { return server.EmbeddedRunnerStatus().Ready }, 5*time.Second, 10*time.Millisecond)
	runnerID := server.EmbeddedRunnerStatus().RunnerID
	_, err := server.runnerRegistry.OpenRun(t.Context(), runnerID, protocol.RunOpenParams{RunID: "stalled-run", ConversationID: "stalled-conversation"})
	require.NoError(t, err)
	ctx, cancel := context.WithCancel(t.Context())
	defer cancel()
	done := make(chan error, 1)
	go func() {
		_, err := server.runnerRegistry.ExecuteTool(ctx, runnerpayload.ToolExecuteParams{RunID: "stalled-run", ToolCallID: "generate", Name: "generate_test_image", Input: json.RawMessage(`{}`)}, nil)
		done <- err
	}()
	select {
	case <-started:
	...
	runner, found := server.runnerRegistry.Runner(runnerID)
	require.True(t, found)
	raw, err := json.Marshal(runnerpayload.ArtifactRequest{RunID: "stalled-run", ToolCallID: "generate", Attachment: tooltypes.ToolAttachment{Type: "image"}})
	require.NoError(t, err)
	value, rpcErr := server.HandleRunnerArtifactRequest(t.Context(), runnerregistry.UIRequestIdentity{RunnerID: runnerID, ConnectionID: runner.ConnectionID, Generation: runner.Generation}, runnerpayload.MethodArtifactUpload, raw)
	...
	_, err = fmt.Fprintf(conn, "PUT %s HTTP/1.1\r\nHost: test\r\nAuthorization: Bearer %s\r\nContent-Length: 16\r\n\r\nx", runnerpayload.ArtifactUploadPath, grant.Token)
	require.NoError(t, err)
