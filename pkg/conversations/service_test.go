...
func TestConversationService_ForkConversation(t *testing.T) {
	...
	t.Run("successful fork", func(t *testing.T) {
		...
		require.True(t, ok)
		assert.Equal(t, sourceRecord.ID, forkMetadata["source_conversation_id"])
		assert.Equal(t, sourceRecord.ID, forkMetadata["root_conversation_id"])
		assert.Equal(t, 1, forkMetadata["depth"])
		assert.Equal(t, string(conversations.ConversationForkModeStoredCopy), forkMetadata["mode"])
