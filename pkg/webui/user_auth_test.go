...
func TestUserLoginHTTPApprovalLifecycleAndTrustedVerificationURL(t *testing.T) {
	...
	assert.Equal(t, "https", complete.Scheme)
	assert.Equal(t, "trusted.example:8443", complete.Host)
	assert.Equal(t, userauth.DeviceVerificationPath, complete.Path)
	assert.Equal(t, started.UserCode, complete.Query().Get("user_code"))
	assert.Empty(t, complete.Query().Get("tenant"))
	assert.NotContains(t, started.VerificationURLComplete, "attacker.example")
	...
	assert.Contains(t, pageResponse.Body.String(), started.UserCode)
	assert.Contains(t, pageResponse.Body.String(), "kodelet")
	assert.Contains(t, pageResponse.Body.String(), "linux/amd64")
	assert.Contains(t, pageResponse.Body.String(), "v-test")
	assert.Contains(t, pageResponse.Body.String(), ">Approve<")
	assert.Contains(t, pageResponse.Body.String(), ">Deny<")
	...
	wrongOriginResponse := performUserLoginDecision(t, server, started.UserCode, "approve", sessionToken, csrfToken, csrfToken, "https://attacker.example", "")
	...
	wrongCSRFResponse := performUserLoginDecision(t, server, started.UserCode, "approve", sessionToken, csrfToken, "wrong-csrf-token", "https://trusted.example:8443", "")
