...
const ChatPage: React.FC = () => {
	...
	const [locallyRunningConversationIds, setLocallyRunningConversationIds] =
		useState<string[]>([]);
	...
	const workspaceOverlayLayout = useMediaQuery(
		WORKSPACE_OVERLAY_MEDIA_QUERY,
	);
	...
	const runningConversationIds = useMemo(
		() => {
			...
			return Array.from(
				new Set([...runningIds, ...locallyRunningConversationIds]),
			);
		},
	...
	useEffect(() => {
		...
		const handleKeyDown = (event: KeyboardEvent) => {
			...
			const focusableElements = Array.from(
				newChatDialogRef.current.querySelectorAll<HTMLElement>(
		...
		return () => {
			...
			window.setTimeout(() => {
				...
				document
					.querySelector<HTMLElement>("button.composer-inline-context")
					?.focus();
			}, 0);
		};
	}, [newChatDialogOpen]);
	...
	useEffect(() => {
		...
		const previousFocus =
			document.activeElement instanceof HTMLElement
				? document.activeElement
				: null;
		const returnFocus = sidebarOverlayOpen
	...
	useEffect(() => {
		...
		const handleTerminalExitKey = (event: KeyboardEvent) => {
			...
			const terminalHost = workspace.querySelector<HTMLElement>(
	...
	}, [clearRunningConversation, conversationId, conversationLoading, loadedConversationId, markConversationRunning]);
									...
									upsertConversationSummary(
										previousStreamedId && previousStreamedId !== streamedId
											? currentConversations.filter(
													(currentConversation) =>
														currentConversation.id !== previousStreamedId,
												)
