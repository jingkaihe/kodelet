  ...
  const commands: ChatRenderToolCall[] = [
    {
      ...
      input: JSON.stringify({
        ...
        description: 'Inspect the transcript changes',
      }),
      result: {
        toolName: 'bash',
        success: true,
    ...
    },
    {
      ...
      input: JSON.stringify({
        ...
        description: 'Check transcript behavior',
      }),
      inProgress: running,
      result: {
        toolName: 'bash',
        ...
        metadata: {
          ...
          output: failed
            ...
            : running
              ? '✓ ChatTranscript.test.tsx (28 tests)'
              : 'Test files  2 passed · Tests  46 passed',
          exitCode: failed ? 1 : 0,
    ...
    activities: [
      {
        ...
        sequence: 1,
        kind: 'search',
        label: 'Located tool and thought renderers',
        status: 'succeeded',
      },
      {
        id: 'search-progress',
        sequence: 2,
        kind: 'read',
        label: 'Check live-to-completed transitions',
  ...
  const extensions: ChatRenderToolCall[] = [
    {
      ...
      result: {
        ...
        metadata: {
          ...
          toolName: 'code_search',
          output: running
            ? 'Tracing tool progress…'
            : "Found the grouping boundary in **ChatToolActivity**. Live results preserve each tool's output.",
          data: { taskRun },
        },
      },
    },
  ...
  return [
    {
      role: 'user',
      content:
        'Give the transcript a TUI feel: collapse finished tools and thoughts, but keep live progress visible.',
    },
    {
      role: 'assistant',
      blocks: [
        {
          type: 'thinking',
          content: 'Group adjacent built-in tools without changing the order of the conversation.',
        },
        {
          type: 'thinking',
          content:
            'Keep extension presentations and live progress intact, then collapse successful groups.',
        },
        {
          type: 'tools',
          tools: [...commands, readTranscript, patch, ...(failed ? [] : extensions)],
        },
        ...(running
          ? []
          : [
...
export const MixedActivitySpacing: Story = {
        ...
        blocks: [
            ...
            tools: [
              {
                ...
                name: 'todo_write',
                input: '{}',
                result: { toolName: 'todo_write', success: true },
              },
              {
                callId: 'spacing-bash',
                name: 'bash',
                ...
                result: {
                  toolName: 'bash',
                  success: true,
                  metadata: { command: 'pwd', output: '/workspace/kodelet', exitCode: 0 },
                },
              },
            ],
          },
          { type: 'thinking', content: 'Check the spacing across tool block boundaries.' },
          {
            ...
            tools: [
              {
                callId: 'spacing-mcp',
                name: 'mcp__maco_code_execute',
                input: '{}',
                result: {
                  toolName: 'mcp__maco_code_execute',
                  metadataType: 'extension_tool',
                  success: true,
                  metadata: { toolName: 'mcp__maco_code_execute', output: '{"ok":true}' },
                },
              },
              readTranscript,
            ],
  ...
  play: async ({ canvasElement }) => {
    ...
    const rows = Array.from(
      canvasElement.querySelectorAll<HTMLDetailsElement>('.activity-stack > details')
    );
    ...
    const paragraphGap = Number.parseFloat(
      getComputedStyle(canvasElement.ownerDocument.documentElement).fontSize
    );
    expect(prose.getBoundingClientRect().top - rows[6].getBoundingClientRect().bottom).toBeCloseTo(
      paragraphGap,
      1
    );
    expect(rows[7].getBoundingClientRect().top - prose.getBoundingClientRect().bottom).toBeCloseTo(
      paragraphGap,
      1
    );
  ...
  render: function Render(args) {
    ...
    return (
      <>
        <div
          role="toolbar"
          aria-label="Transcript lifecycle"
          className="mx-auto flex max-w-5xl gap-3 px-3 pt-3 sm:px-4 md:px-8"
        >
          <button
            type="button"
          ...
          <output className="self-center font-mono text-xs text-kodelet-dark">
        ...
        <ChatTranscript
          ...
          messages={finished ? terminalMessages('completed') : args.messages}
        />
      </>
    );
  },
