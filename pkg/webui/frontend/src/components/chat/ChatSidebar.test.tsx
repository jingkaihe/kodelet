...
describe('ChatSidebar hierarchy', () => {
  ...
  it('preserves child selection, running indicators and per-conversation actions', () => {
    ...
    expect(rows.map((row) => row.dataset.testid)).toEqual([
      ...
      'conversation-row-child',
      'conversation-row-orphan',
    ]);
    ...
    expect(
      within(screen.getByTestId('conversation-row-orphan')).getByText('Child')
    ).toBeInTheDocument();
  });
  ...
  it('keeps whole trees together at Show more boundaries and reveals active descendants', () => {
    ...
    const { rerender } = render(
      <ChatSidebar {...props} activeConversationId={null} conversations={conversations} />
    );
    ...
    expect(screen.getAllByTestId(/^conversation-row-/)).toHaveLength(13);
    rerender(
      <ChatSidebar {...props} activeConversationId="last-child" conversations={conversations} />
    );
...
describe('ConversationSearchDialog', () => {
  it('focuses search and exposes search, workspace, and result actions', async () => {
    ...
    expect(screen.getByRole('button', { name: /Alpha conversation/i })).toHaveTextContent('conv-a');
  ...
  it('expands the group containing a newly active conversation', async () => {
    ...
    const { rerender } = render(<ChatSidebar activeConversationId={null} {...props} />);
