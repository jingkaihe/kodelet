...
const meta = {
  ...
  beforeEach: () => {
    ...
    const copilotLogin = {
      id: 'storybook-copilot',
      status: 'pending' as const,
      verificationUrl: 'https://github.com/login/device',
      userCode: 'WXYZ-2345',
    };
    const mocks = [
      spyOn(apiService, 'getCodexProviderStatus').mockResolvedValue({
        provider: 'codex',
        connected: true,
      }),
      spyOn(apiService, 'getCopilotProviderStatus').mockResolvedValue({
        provider: 'copilot',
        connected: true,
      }),
      spyOn(apiService, 'getAnthropicProviderStatus').mockResolvedValue({
        provider: 'anthropic',
        connected: true,
      }),
...
export const DeviceSignIn: Story = {
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.findByLabelText('ChatGPT device code')).resolves.toHaveTextContent(
      'ABCD-EFGH'
    );
  },
};
...
export const AuthorizationCode: Story = {
  play: async ({ canvasElement }) => {
    ...
    await waitFor(() =>
      expect(canvas.getByRole('textbox', { name: 'Anthropic authorization code' })).toBeVisible()
    );
  },
};
...
export const FailedSignIn: Story = {
  beforeEach: () => {
    const mock = spyOn(apiService, 'startCodexDeviceLogin').mockResolvedValue({
      id: 'storybook-codex',
      status: 'failed',
      message: 'The device code expired. Start a new sign-in.',
    });
