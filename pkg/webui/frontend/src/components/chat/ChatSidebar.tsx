...
const isOIDCPrincipal = (principal: AuthPrincipal | null | undefined): principal is AuthPrincipal =>
...
export const ConversationSearchDialog: React.FC<ConversationSearchDialogProps> = ({
  ...
  const resultCountLabel = initialLoading
    ? '…'
  ...
  return (
    // biome-ignore lint/a11y/noStaticElementInteractions: Clicking only the backdrop dismisses the dialog; the close button and Escape provide keyboard dismissal.
    <div
...
export const groupConversationsByCwd = (conversations: Conversation[]) => {
  ...
  for (const conversation of conversations) {
    ...
    const parent =
      conversation.metadata?.parent_conversation_id ?? conversation.parentConversationId;
  ...
  const byActivity = (left: ConversationTreeNode, right: ConversationTreeNode) =>
  ...
  return Array.from(groups.values()).sort(
    (left, right) => right.latestActivity - left.latestActivity
  );
};
...
const ChatSidebar: React.FC<ChatSidebarProps> = ({
  ...
  const [visibleGroupCounts, setVisibleGroupCounts] = React.useState<Record<string, number>>({});
  ...
  return (
    <aside className="chat-sidebar-surface relative flex h-full flex-col overflow-visible border-b border-black/8 px-4 py-4 lg:border-b-0">
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        <div aria-busy={loading} className="conversation-list min-h-0 flex-1 overflow-y-auto pr-1">
          ...
          {showLoadingState ? (
            <div className="px-2 py-2 text-sm text-kodelet-dark/65">Loading…</div>
          ) : null}
            ...
            <section className="conversation-group" key={group.key}>
              {(() => {
                const isExpanded =
                  ...
                  isGroupExpandedByDefault(group, groupIndex, activeConversationId);
                const minimumVisibleCount = minimumConversationCount(group, activeConversationId);
                ...
                return (
                  <>
                    ...
                    >
                      ...
                      <span className="conversation-group-labels" title={group.cwd || group.label}>
                        <span className="conversation-group-title">{group.primaryLabel}</span>
                        {group.secondaryLabel ? (
                          <span className="conversation-group-path">{group.secondaryLabel}</span>
                        ) : null}
                      </span>
                              ...
                              {conversation.depth > 0 ? (
                                <span aria-hidden="true" className="conversation-tree-guide">
                                  {conversation.ancestorGuides.map((continued, index) => (
                                    <span
                                      // biome-ignore lint/suspicious/noArrayIndexKey: Each decorative guide represents a fixed ancestor depth, not a reorderable conversation.
                                      key={index}
                                      className={cn(
                                  ...
                                  <span
                                    className={cn(
                                      'conversation-branch',
                                      conversation.isLastChild && 'last-child'
                                    )}
                                  />
                                </span>
                              ) : null}
                              <button
                                ...
                                title={
                                  conversation.parentId
                                    ? `Child of ${conversation.parentId}`
                                    : undefined
                                }
                                type="button"
                              >
                                ...
                                {conversation.parentId && conversation.depth === 0 ? (
                                  <span
                                    className="conversation-orphan-label"
                                    title="Parent is not available in this list"
                                  >
                                    Child
                                  </span>
                                ) : null}
