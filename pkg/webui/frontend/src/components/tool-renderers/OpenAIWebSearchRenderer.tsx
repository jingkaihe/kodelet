...
import type { OpenAIWebSearchMetadata, ToolResult } from '../../types';
...
const stringArrayField = (source: Record<string, unknown> | null, ...keys: string[]): string[] => {
  for (const key of keys) {
    const value = source?.[key];
...
const OpenAIWebSearchRenderer: React.FC<OpenAIWebSearchRendererProps> = ({
  toolResult,
  toolInput,
}) => {
  ...
  const missingTargetMessage =
    !primaryTarget && actionType === 'open_page'
  ...
  const links = uniqueItems([...(meta?.sources || []), ...(meta?.results || [])]).filter(
    (link) => link !== url
  );
      ...
      {visibleLinks.length > 0 ? (
        <div className="flex flex-col gap-2">
          ...
          {links.length > visibleLinks.length ? (
            <ReferenceToolNote
              text={`Showing first ${visibleLinks.length} of ${links.length} links.`}
            />
          ) : null}
        </div>
      ) : null}
