...
// ConfiguredOpenAITextVerbosity returns an explicitly configured Responses API
// text verbosity. The boolean is false when the setting is absent, allowing
// the upstream API to apply its default.
...
// configured Responses API text verbosity while preserving the unset state.
