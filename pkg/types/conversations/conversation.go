...
type ConversationRecord struct {
	...
	CompactionHistory *CompactionHistory                    `json:"compactionHistory,omitempty"`
...
func ForkConversationRecordWithOptions(source ConversationRecord, options ConversationForkOptions) ConversationRecord {
	...
	forked.CompactionHistory = source.CompactionHistory.Clone()
...
func (cr *ConversationRecord) ToSummary() ConversationSummary {
	firstMessages := cr.RawMessages
	if history := cr.CompactionHistory; history != nil && len(history.Segments) > 0 {
		firstMessages = history.Segments[0].RawMessages
	}
	...
	if len(firstMessages) > 0 {
		...
		if err := json.Unmarshal(firstMessages, &messages); err == nil && len(messages) > 0 {
	...
	if history := cr.CompactionHistory; history != nil {
		if visible, err := history.VisibleMessages(cr.RawMessages); err == nil {
			messageCount = strings.Count(string(visible), `"role"`)
			for _, segment := range history.Segments {
				messageCount += strings.Count(string(segment.RawMessages), `"role"`)
			}
		}
	}
