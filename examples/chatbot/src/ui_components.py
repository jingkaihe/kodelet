...
from kodelet_api import KodeletConversation
...
def render_sidebar(conversation_manager: ConversationManager) -> Optional[str]:
    ...
    for conversation in conversations:
        # Create a container for each conversation with improved styling
        ...
        with container:
            ...
            # Create display text with very aggressive truncation for sidebar
            provider_icon = "🤖" if conversation.provider == "anthropic" else "🧠"

            # Truncate preview extremely aggressively to ensure it fits
            if len(preview) > 25:
                preview = preview[:22] + "..."
            ...
            # Use single full-width conversation button
            if st.button(
                display_text,
                key=f"conv_{conversation.id}",
                help=f"ID: {conversation.id[:8]}...\nUpdated: {time_str}\nMessages: {conversation.message_count}",
                use_container_width=True,
                type=button_type
            ):
                selected_conversation_id = conversation.id

            # Small delete button in a centered layout
    ...
    # Add status information at the bottom of sidebar
    st.sidebar.markdown("---")
    st.sidebar.subheader("Status")

    # Status panel in sidebar with improved styling
    with st.sidebar:
        # API and conversation status in a compact layout
